...
class SimulationHistoryBuffer:
    ...
    def __init__(
        ...
        :param ego_state_buffer: Past ego state trajectory including the state. at the current time step [t_-N, ..., t_-1, t_0]
        :param observations_buffer: Past observations including the observation. at the current time step [t_-N, ..., t_-1, t_0].
        ...
        if len(ego_state_buffer) != len(observations_buffer):
            raise ValueError("Ego and observations buffer is " f"not the same length {len(ego_state_buffer) != len(observations_buffer)}!")
