...
class IDMAgentManager:
    ...
    def propagate_agents(self, ego_state: EgoState, tspan: float, iteration: int, open_loop_detections: List[TrackedObject], radius: float) -> None:
        """
        ...
        for track in open_loop_detections:
            ...
            self.agent_occupancy.insert(geometry_id=track.track_token, geometry=track.box.geometry)

        self._filter_agents_out_of_range(ego_state=ego_state, radius=radius)
        ...
        for agent_token, agent in self.agents.items():
            if agent.is_active(iteration) and agent.has_valid_path():
                ...
                agent_path = path_to_linestring(path=agent.get_path_to_go())
                # (agent.width / 2 * 1.1)
                intersecting_agents = self.agent_occupancy.intersects(
                    geometry=agent_path.buffer(distance=(agent.width * 0.55), cap_style=CAP_STYLE.flat)
                )
                ...
                if intersecting_agents.size > 1:
                    nearest_id, nearest_agent_polygon, relative_distance = intersecting_agents.get_nearest_entry_to(geometry_id=agent_token)
                    ...
                    projected_velocity = rotate_angle(pose=StateSE2(longitudinal_velocity, 0, 0), theta=relative_heading).x
                ...
                agent.propagate(
                    lead_agent=IDMLeadAgentState(progress=relative_distance, velocity=projected_velocity, length_rear=length_rear),
                    tspan=tspan,
                )
                self.agent_occupancy.set(geometry_id=agent_token, geometry=agent.projected_footprint)
    ...
    def get_active_agents(self, iteration: int, num_samples: int, sampling_time: float) -> DetectionsTracks:
        ...
        """
        tracked_objects_agents = [
            agent.get_agent_with_planned_trajectory(num_samples=num_samples, sampling_time=sampling_time)
            for agent in self.agents.values()
            if agent.is_active(iteration=iteration)
        ]
        tracked_objects = TrackedObjects(tracked_objects=tracked_objects_agents)
        detections_tracks = DetectionsTracks(iteration_step=iteration, is_real_data=False, tracked_objects=tracked_objects)
