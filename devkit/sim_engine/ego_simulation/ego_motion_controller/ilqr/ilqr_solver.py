...
from devkit.sim_engine.ego_simulation.ego_motion_controller.tracker_utils import (
...
class ILQRSolverParameters:
    ...
    `ILQRSolverParameters` 类是一个用于表示 iLQR (Iterative Linear Quadratic Regulator) 求解器参数的不可变（frozen）数据类.
    #### **1. 类和字段说明**
    - **`discretization_time`**: 时间离散化，用于积分。
    - **`state_cost_diagonal_entries` 和 `input_cost_diagonal_entries`**: 状态和输入的成本加权矩阵，对应于 [x, y, heading, velocity, steering angle] 和 [acceleration, steering rate]。
    - **`state_trust_region_entries` 和 `input_trust_region_entries`**: 线性化信赖区域的加权矩阵，帮助控制每次更新步骤的线性化误差。
    - **`max_ilqr_iterations`**: iLQR的最大迭代次数。
    - **`convergence_threshold`**: 迭代过程中，delta输入小于该阈值时提前终止。
    - **`max_solve_time`**: 如果定义了最大求解时间，则在达到时间限制后终止iLQR求解。
    - **`min_velocity_linearization`**: 线性化时，低于该速度阈值时，速度被修改为该阈值。
    """
    ...
    max_solve_time: Optional[float]  # [s] If defined, sets a maximum time to run a solve call of iLQR before terminating.
    ...
    def __post_init__(self) -> None:
        ...
        #### **2. `__post_init__` 校验函数**
        `__post_init__` 函数在类实例化后自动执行，用于校验一些重要字段的有效性：
        - **正值校验**：校验 `discretization_time`, `max_ilqr_iterations`, `convergence_threshold`, `min_velocity_linearization` 是否为正值。
        - **`max_solve_time` 校验**：如果定义了 `max_solve_time`，确保其为正值。
        - **矩阵的正定性校验**：通过校验状态和输入的成本矩阵是否满足正半定和正定的要求。
        """
        for entry in ["discretization_time", "max_ilqr_iterations", "convergence_threshold", "min_velocity_linearization"]:
            assert getattr(self, entry) > 0.0, f"#log# Field {entry} should be positive."
    ...
    def init_ego_vehicle_parameters(self, mine_name: str):
        self.vehicle: VehicleParameters = get_mine_truck_parameters(mine_name=mine_name)
        self.wheelbase = self.vehicle.wheel_base
        # Constraints for underlying dynamics model. # 底层动力学模型的约束。
        self.max_acceleration = self.vehicle.ilqr_tracker_constraints["max_acceleration"]
        self.max_steering_angle = self.vehicle.ilqr_tracker_constraints["max_steering_angle"]
        self.max_steering_angle_rate = self.vehicle.ilqr_tracker_constraints["max_steering_angle_rate"]
...
class ILQRSolver:
    ...
    def get_ego_vehicle_parameters(self, mine_name: str):
        self.vehicle: VehicleParameters = get_mine_truck_parameters(mine_name=mine_name)
        self._wheel_base = self.vehicle.wheel_base
        self._solver_params.init_ego_vehicle_parameters(mine_name=mine_name)
        ...
        max_acceleration = self.vehicle.ilqr_tracker_constraints["max_acceleration"]
        max_steering_angle_rate = self.vehicle.ilqr_tracker_constraints["max_steering_angle_rate"]
        self._input_clip_min = (-max_acceleration, -max_steering_angle_rate)
        self._input_clip_max = (max_acceleration, max_steering_angle_rate)
