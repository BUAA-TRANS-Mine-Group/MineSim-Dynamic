...
from devkit.common.actor_state.vehicle_parameters import VehicleParameters, get_mine_truck_parameters
...
class LQRTracker(AbstractTracker):
    ...
    def track_trajectory(
        ...
        initial_velocity, initial_lateral_state_vector = self._compute_initial_velocity_and_lateral_state(
            current_iteration=current_iteration, initial_state=initial_state, trajectory=trajectory
        )
    ...
    def _lateral_lqr_controller(
        ...
        assert len(velocity_profile) == self._tracking_horizon, (
            f"The linearization velocity sequence should have length {self._tracking_horizon} " f"but is {len(velocity_profile)}."
        )
        assert len(curvature_profile) == self._tracking_horizon, (
            f"The linearization curvature sequence should have length {self._tracking_horizon} " f"but is {len(curvature_profile)}."
        )
