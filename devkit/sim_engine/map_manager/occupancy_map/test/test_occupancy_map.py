...
from devkit.sim_engine.map_manager.occupancy_map.geopandas_occupancy_map import GeoPandasOccupancyMapFactory
...
class OccupancyMapTests(unittest.TestCase):
    ...
    def test_intersects_polygon(self):  # type: ignore
        ...
        strtree_occupancy_map = STRTreeOccupancyMapFactory.get_from_geometry(geometries=[self.p1])

        def test(occupancy_map: OccupancyMap) -> None:
            intersection = occupancy_map.intersects(geometry=self.p2)
            assert not intersection.is_empty()
            intersection = occupancy_map.intersects(geometry=self.p3)
    ...
    def test_intersects_linestring(self):  # type: ignore
        ...
        def test(occupancy_map: OccupancyMap) -> None:
            intersection = occupancy_map.intersects(geometry=self.l1)
            assert intersection.is_empty()
            occupancy_map.insert(geometry_id="2", geometry=self.p2)
            intersection = occupancy_map.intersects(geometry=self.l1)
            assert not intersection.is_empty()
    ...
    def test_intersects_linestring_buffered(self):  # type: ignore
        ...
        def test(occupancy_map: OccupancyMap) -> None:
            intersection = occupancy_map.intersects(self.l1.buffer(distance=0.4, cap_style=2))
            assert intersection.is_empty()
            occupancy_map.insert(geometry_id="4", geometry=self.p4.buffer(distance=0.5, cap_style=2))
            intersection = occupancy_map.intersects(self.l1.buffer(distance=0.1, cap_style=2))
            assert intersection.is_empty()
            occupancy_map.insert(geometry_id="2", geometry=self.p2)
            intersection = occupancy_map.intersects(self.l1.buffer(distance=0.4, cap_style=2))
            assert not intersection.is_empty()
    ...
    def test_insert_get_set(self):  # type: ignore
        ...
        def test(occupancy_map: OccupancyMap) -> None:
            ...
            occupancy_map.insert(geometry_id="2", geometry=self.p3)
            assert occupancy_map.size == 2
            assert self.p3 == occupancy_map.get(geometry_id="2")
            occupancy_map.set("2", self.p2)
            assert self.p2 == occupancy_map.get(geometry_id="2")
