...
from devkit.sim_engine.map_manager.minesim_map_data.minesim_semanticmap_json_loader import semantic_map_hashes

Image.MAX_IMAGE_PIXELS = None
Axis = Any


class MineSimBitMapPngLoader:
    ...
    def __init__(
        self,
        map_version: str = "v1.5",
        map_root: str = "/home/czf/project_czf/20240901-MineSim/MineSim-Dynamic-Dev-czf/datasets/maps",
        ...
        Args:
            map_root (str,optional):map_root根目录. Defaults to
    ...
    def load_bitmap_info(self):
        """load bitmap information.加载地理坐标信息 and 像素坐标信息；"""
    ...
    def load_bitmap(self, is_transform_gray=False) -> np.ndarray:
        ...
        if self.bitmap_type == "bitmap_mask":
            bitmap_hashes = {"jiangxi_jiangtong": "jiangxi_jiangtong_bitmap_mask", "guangdong_dapai": "guangdong_dapai_bitmap_mask"}
            bitmap_hash = bitmap_hashes[self.location]
            dir_bitmap = os.path.join(self.map_root, "bitmap", bitmap_hash + ".png")
            if os.path.exists(dir_bitmap):
                ...
                # 在图像处理和绘制中，通常图像的坐标系和实际的地理坐标系是不同的。图像坐标系的原点通常在左上角，而地理坐标系的原点通常在左下角。
                # 这就导致在直接绘制图像时会出现 y 轴颠倒的现象。通过使用 np.flipud 函数翻转图像的 y 轴，可以将图像的原点从左上角转换到左下角，使其与地理坐标系对齐。
                # 因此，虽然图像被翻转了，但绘制时与地理坐标系的方向一致。
            else:
                raise Exception("###Exception### %s的地图路径不存在 %s! Please check." % (self.bitmap_type, dir_bitmap))

        elif self.bitmap_type == "bitmap_rgb":
            bitmap_hashes = {"jiangxi_jiangtong": "jiangxi_jiangtong_bitmap_rgb", "guangdong_dapai": "guangdong_dapai_bitmap_rgb"}
            bitmap_hash = bitmap_hashes[self.location]
            dir_bitmap = os.path.join(self.dataroot, "maps", "bitmap", bitmap_hash + ".png")
            if os.path.exists(dir_bitmap):
    ...
    def load_bitmap_using_utm_local_range(
        ...
        Args:
            ...
            x_margin (int, optional): x外边缘尺寸. Defaults to 10.
            y_margin (int, optional): y外边缘尺寸. Defaults to 10.
        ...
        # calculate pixel
        px_min, py_min = self._utm_to_pixel(
            utm_local_range[0] - x_margin, utm_local_range[1] - y_margin
        )  # 注意像素坐标与UTM坐标在y轴上的方向映射是正确的
        ...
        return self.image_ndarray_local
    ...
    def render_mask_map_using_image_ndarray_local(self, ax: Axis = None, window_size=4, gray_flag=True, zorder: int = 2):
        """渲染可行驶区域二值化栅格图(局部);
        注:黑白图 和 灰色图 两种.
        ...
        if gray_flag == True:  # 黑白图
            ax.imshow(image_ndarray_local, extent=[x_min, x_max, y_min, y_max], cmap="gray", origin="lower", zorder=zorder)
            ...
            ax.imshow(image_ndarray_local, extent=[x_min, x_max, y_min, y_max], cmap=cm, origin="lower", zorder=zorder)
    ...
    def downsample_with_filter(image: np.ndarray = None, window_size: int = 5) -> np.ndarray:
        ...
        # 二值化
        downsampled_binary = (downsampled > 0.5).astype(bool)  # 或者 .astype(np.bool_)
    ...
    def render_mask_map(self, ax: Axis = None, gray_flag: bool = True):
        ...
        x_min, x_max = self.bitmap_info["bitmap_mask_PNG"]["UTM_info"]["local_x_range"]
        y_min, y_max = self.bitmap_info["bitmap_mask_PNG"]["UTM_info"]["local_y_range"]
    ...
    def render_rgb_map(self, ax: Axis = None, zorder: int = 2):
        ...
        if len(self.image_ndarray.shape) == 2:
            ax.imshow(self.image_ndarray, extent=[0, x, 0, y], cmap="gray", zorder=zorder)
        else:
            ax.imshow(self.image_ndarray, extent=[0, x, 0, y], zorder=zorder)
