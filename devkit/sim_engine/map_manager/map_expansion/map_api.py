...
class TgScenesMap:
    ...
    def __init__(self, dataroot: str = "/home/czf/project_czf/20231010_onsite_mine/datasets", location: str = "jiangxi_jiangtong"):
        ...
        # if self.location == "jiangxi_jiangtong" and self.version != "1.5":
        #     raise Exception("Error:只针对地图版本为 map version 1.5! 你的地图版本是", self.version)
        # if self.location == "guangdong_dapai" and self.version != "1.6":
        #     raise Exception("Error:只针对地图版本为 map version 1.6! 你的地图版本是", self.version)

        # load json data
        self._load_layers()  # 加载各个图层
        self._make_token2ind()  # token标识符转化为id: 0,1,2, ...... ;
        self.explorer = TgScenesMapExplorer(self)

        logger.info(f"#log# Load open-pit HD map, location={self.location}, version={self.version}!")
    ...
    def _get_start_dubinspose_of_all_intersection(
        self,
    ):
        pass

    # TODO
    def get_outgoing_plogon_token(
        self,
    ):
        pass

    # TODO
    def get_outgoing_dubinspose_token(
        self,
    ):
        pass
...
class TgScenesMapExplorer:
    ...
    def __init__(
        ...
        self.canvas_min_x = 0
        self.canvas_max_y = self.map_api.semantic_map_json["bitmap_rgb_PNG"]["canvas_edge_meter"][0]
        self.canvas_min_y = 0
    ...
    def extract_polygon(self, polygon_token: str) -> Polygon:
        ...
        link_coords = [(self.map_api.get("node", token)["x"], self.map_api.get("node", token)["y"]) for token in polygon_record["link_node_tokens"]]
    ...
    def _render_polygon_layer(
        ...
        if tokens is not None:
            ...
            for record in records:  # 一个token一个的画图
                ...
                ax.add_patch(
                    descartes.PolygonPatch(polygon, fc=face_color_with_alpha, ec=edge_color_with_alpha, label=label, zorder=zorder, lw=linewidth)
                )
                # ax.add_patch(descartes.PolygonPatch(polygon,fc=self.color_map[layer_name],alpha=alpha,label=label) #!之前的代码

    def _render_polygon_tokens_layer(self, ax: Axes, alpha: float, tokens: List[str] = None, zorder: int = 2) -> None:
        """
        Renders an individual non-geometric polygon layer on an axis.
        :param ax:The matplotlib axes where the layer will get rendered.
        :param alpha:The opacity of the layer to be rendered.
        ...
        for token in tokens:  # 一个token一个的画图
            polygon = self.map_api.extract_polygon(token)  # 创建一个 Shapely 多边形对象
            ...
            ax.add_patch(descartes.PolygonPatch(polygon, fc=face_color_with_alpha, ec=edge_color_with_alpha, label=token, zorder=zorder))
    ...
    def _render_road_block_polygon_layer(
        ...
        if tokens is not None:
            ...
            for record in records:  # 一个token一个的画图
                ...
                link_coords = [
                    (self.map_api.get("node_block", token)["x"], self.map_api.get("node_block", token)["y"]) for token in link_nodeblock_tokens
    ...
    def _render_reference_path_centerlines(
        ...
        for token in tokens:
            ...
            reference_path = self.map_api.reference_path[id_path]
            if "waypoints" not in reference_path:
                continue
            waypoints = reference_path["waypoints"]
            # Resampling
    ...
    def render_edge_borderlines(
        ...
        #todo
        """
        for id_borderline, borderline in enumerate(self.map_api.borderlines):
            pass
