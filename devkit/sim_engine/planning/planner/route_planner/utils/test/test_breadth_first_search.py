...
start_refpath_token, start_refpath_waypoint_id = calculate_start_refpath_info(
    start_pose_statese2=StateSE2(x=1570.55, y=701.404, heading=6.01),
    semantic_map=mine_map_api.semantic_map,
)

mine_search_bfs = BreadthFirstSearch(
    minesim_graph=minesim_graph,
    start_refpath_token=start_refpath_token,
    start_refpath_waypoint_id=start_refpath_waypoint_id,
)
path_tokens, is_search_successful = mine_search_bfs.search_route_path(goal_task=goal_task, target_depth=5)


minesim_graph.search_route_path
