...
@dataclass
class PredefinedManeuverModeSamplingPlannerSettings(abc.ABC):
    """Parameters related to the renetOptimalPlanner implementation."""

    num_width: int = (3,)  # road width方向的 sampling number
    num_jerk: int = (5,)  # jerk sampling number

    max_road_width: Optional[float] = 1.4  # maximum road width [m]
    highest_speed: Optional[float] = 9.0  # highest speed [m/s]
    highest_jerk: Optional[float] = 10.0  # highest sampling jerk [m/s^3]
    lon_accel_max: float = 8.2  #  [m/s^2]
    lon_accel_min: float = -8.2  #  [m/s^2]

    def __post_init__(self) -> None:
        assert self.num_width > 0, "num_width must be positive."
        assert self.num_jerk > 0, "num_speed must be positive."


class PredefinedManeuverModeSamplingPlanner(AbstractPlanner):
    """
    Sampling Planner using Predefined Maneuver Modes (SPPMM)
    """
    ...
    def __init__(
        self,
        planner_settings: PredefinedManeuverModeSamplingPlannerSettings,
        ...
        self.jerk_values_sampling = np.linspace(-self.settings.highest_jerk, self.settings.highest_jerk, self.settings.num_jerk)

        if self.requires_init_ego_vehicle_parameters:
            self.ego_vehicle: VehicleParameters = scenario.ego_vehicle_parameters
            self.settings.highest_speed = self.ego_vehicle.constraints["max_longitudinal_velocity"] * 0.8
        ...
        self.cost_function = JerkSpaceSamplingCostFunction("WX1", vehicle_info=self.ego_vehicle_commonroad, max_road_width=max_road_width)
        ...
        self.best_traj: JerkSpaceSamplingFrenetTrajectory = None
        self.all_trajs_before_check: List[JerkSpaceSamplingFrenetTrajectory] = []  # 记录最后保留下来的所有轨迹,碰撞检测前
        self.all_trajs: List[JerkSpaceSamplingFrenetTrajectory] = []  # 记录每个时间步的最佳轨迹
    ...
    def _get_planned_trajectory(self, ego_state_nuplan: EgoState, best_traj: JerkSpaceSamplingFrenetTrajectory) -> InterpolatedTrajectory:
    ...
    def sampling_frenet_trajectories_using_sppmm(self, frenet_state: FrenetState) -> list:
        """预定义机动模式采样策略"""
        frenet_trajectories = []
        traj_per_timestep = []
        ...
        pvt_sampler = PolyVTSampling(
            total_time=self._planned_horizon,
            delta_t=self.dt,
            jerk_min=-self.settings.highest_jerk,
            jerk_max=self.settings.highest_jerk,
            a_min=self.settings.lon_accel_min,
            a_max=self.settings.lon_accel_max,
            v_max=self.settings.highest_speed,
            num_samples=self.settings.num_jerk,
        )
        seeds = pvt_sampler.sampling(s0=frenet_state.s, v0=frenet_state.s_d, a0=frenet_state.s_dd)  # 不同at线形直线拼接采样种子.

        #! lateral sampling
        if self.settings.num_width <= 1:  # 横向不偏移,只瞄准参考路径行驶
            di_list = [0.0]
        else:
            sampling_width = self.settings.max_road_width - self.ego_vehicle.width
            di_list = np.linspace(-sampling_width / 2, sampling_width / 2, self.settings.num_width)  # 横向采样3条路径
        ...
        for path_id, di in enumerate(di_list):
            ...
            lat_qp = QuinticPolynomial(
                ...
                vxe=0.0,
                axe=0.0,
                time=self._planned_horizon,
            )  # 5次多项式曲线插值

            ft.t = np.linspace(0.0, self._planned_horizon, int(round(self._planned_horizon / self.dt)) + 1).tolist()
            ...
            frenet_trajectories = pvt_sampler.get_all_frenet_trajectory(
                s0=frenet_state.s, v0=frenet_state.s_d, a0=frenet_state.s_dd, seeds=seeds, ft=ft
            )
            traj_per_timestep.extend(frenet_trajectories)
    ...
    def calc_global_paths(self, ftlist: list[JerkSpaceSamplingFrenetTrajectory]) -> list:
        """将一系列 JerkSpaceSamplingFrenetTrajectory 对象（代表在 Frenet 坐标系中的路径）转换成全局坐标系中的路径.
        具体来说,它会计算每个 JerkSpaceSamplingFrenetTrajectory 上的全局 x, y 坐标,偏航角,曲率等参数.

        Args:
            ftlist (list[JerkSpaceSamplingFrenetTrajectory]): JerkSpaceSamplingFrenetTrajectory 的列表.其实每次只传入一个值
    ...
    def check_constraints(self, trajs: list[JerkSpaceSamplingFrenetTrajectory]) -> list:
        ...
        Args:
            trajs (list[JerkSpaceSamplingFrenetTrajectory]): JerkSpaceSamplingFrenetTrajectory 的列表.其实每次只传入一个值
    ...
    def has_collision(self, traj: JerkSpaceSamplingFrenetTrajectory, observation_detection: DetectionsTracks, check_resolution: int = 2) -> tuple:
        """检查给定轨迹是否与任何障碍物发生碰撞

        Args:
            traj (JerkSpaceSamplingFrenetTrajectory): 单条轨迹
    ...
    def check_collisions(self, trajs: list, observation_detection: DetectionsTracks) -> list[JerkSpaceSamplingFrenetTrajectory]:
    ...
    def calc_all_trajs_cost(self, trajs: list) -> tuple[PriorityQueue, list]:
        """计算所有轨迹的成本，并返回一个PriorityQueue

        Args:
            trajs (list): 轨迹列表。

        Returns:
            list: 返回 通过cost_total升序排列的队列
        """
        candidate_trajs = PriorityQueue()
        for id_trajs, traj in enumerate(trajs):
            traj = self.cost_function.cost_total(traj=traj, target_speed=self.settings.highest_speed)
            candidate_trajs.put((traj.cost_total, traj, id_trajs))

        return candidate_trajs, trajs

    def plan_trajectory(self, frenet_state: FrenetState, observation_detection: DetectionsTracks) -> JerkSpaceSamplingFrenetTrajectory:
        """frenet 规划方法的核心函数.
        ...
        Returns:
            JerkSpaceSamplingFrenetTrajectory: 规划结果.
        """
        ftlist = self.sampling_frenet_trajectories_using_sppmm(frenet_state=frenet_state)
        ...
        if not ftqueue.empty():
            _, self.best_traj, best_traj_id_ftlist = ftqueue.get()  # 取出并移除优先级最高的任务

        return self.best_traj

    def has_collision_with_boundary(self, traj: JerkSpaceSamplingFrenetTrajectory, check_resolution: int = 10) -> bool:
        """检查给定轨迹是否与道路边界发生碰撞
