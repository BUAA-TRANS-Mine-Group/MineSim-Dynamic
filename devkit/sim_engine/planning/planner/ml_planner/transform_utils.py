...
def _se2_vel_acc_to_ego_state(
    state: StateSE2, velocity: npt.NDArray[np.float32], acceleration: npt.NDArray[np.float32], timestamp: float, vehicle: VehicleParameters
) -> EgoState:
    """
...
def _get_velocity_and_acceleration(
    ...
    timesteps_current_planned_interp = np.arange(start=current_ego_state.time_point.time_s, stop=timesteps[-1] + 1e-6, step=dt)
...
def transform_predictions_to_states(
    ...
    states: list = _get_absolute_agent_states_from_numpy_poses(predicted_poses, ego_history, timesteps)
