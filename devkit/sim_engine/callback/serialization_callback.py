...
def convert_sample_to_scene(
    ...
    planning_problem_goal_task: Optional[StateSE2],
    ...
    # Convert ego pose
    scene["ego"] = dict(to_scene_ego_from_car_footprint(CarFootprint.build_from_center(data.ego_state.center, get_mine_truck_parameters())))
    ...
    trajectories["ego_predicted_trajectory"] = dict(
        to_scene_trajectory_from_list_ego_state(data.trajectory.get_sampled_trajectory(), colors.ego_predicted_trajectory)
    )
    ...
    def on_simulation_end(self, setup: SimulationSetup, planner: AbstractPlanner, history: SimulationHistory) -> None:
        ...
        scenes = [
            convert_sample_to_scene(
                ...
                planning_problem_goal_task=scenario.planning_problem_goal_task,
                data=sample,
