...
def _ray_map_items(task: Task, *item_lists: Iterable[List[Any]], log_dir: Optional[Path] = None) -> List[Any]:
    ...
    assert all(len(cast(List, items)) == len(item_lists[0]) for items in item_lists), "All lists must have equal size"  # type: ignore
    fn = task.fn
    ...
    for object_token, output in tqdm(_ray_object_iterator(object_ids), total=len(object_ids), desc="Ray objects"):
        object_result_map[object_token] = output
