...
from devkit.utils.multithreading.ray_execution import ray_map
from devkit.utils.multithreading.worker_pool import Task, WorkerPool, WorkerResources
...
def initialize_ray(
    ...
    if master_node_ip and use_distributed:
        ...
        logger.info(f"#log# Connecting as part of a cluster at: {master_node_ip} with password: {redis_password}!")
        ...
        number_of_nodes = 1
        logger.info("#log# Starting ray local!")
        ray.init(
...
class RayDistributed(WorkerPool):
    ...
    def initialize(self) -> WorkerResources:
        ...
        if ray.is_initialized():
            logger.warning("#log# Ray is running, we will shut it down before starting again!")
            ray.shutdown()
