...
class WorkerPool(abc.ABC):
    ...
    def __init__(self, config: WorkerResources):
        ...
        logger.info(f"#log# Worker: {self.__class__.__name__}")
        logger.info(f"#log# {self}")
    ...
    def map(self, task: Task, *item_lists: Iterable[List[Any]], verbose: bool = False) -> List[Any]:
        ...
        if verbose:
            logger.info(f"#log# Submitting {max_size} tasks!")
    ...
    def __str__(self) -> str:
        ...
        """
        return (
            f"#log# Number of nodes: {self.config.number_of_nodes}\n"
            f"#log# Number of CPUs per node: {self.config.number_of_cpus_per_node}\n"
            f"#log# Number of GPUs per node: {self.config.number_of_gpus_per_node}\n"
            f"#log# Number of threads across all nodes: {self.config.number_of_threads}"
        )
