...
from devkit.common.actor_state.ego_temporal_state import EgoTemporalState
...
from devkit.common.actor_state.tracked_objects_types import AGENT_TYPES, TrackedObjectType
...
from devkit.utils.color import Color, ColorType
from devkit.utils.serialization.scene import EgoScene, GoalScene, Trajectory, TrajectoryState
...
def to_scene_ego_from_ego_state(ego_pose: Union[EgoState, EgoTemporalState]) -> EgoScene:
    ...
    past = (
        [to_scene_waypoint(state, -current_state.time_point.time_s) for state in ego_temporal_state.past_trajectory.valid_waypoints]
...
def to_scene_from_ego_and_boxes(ego_pose: StateSE2, tracked_objects: TrackedObjects, map_name: str, set_camera: bool = False) -> Dict[str, Any]:
    ...
    scene: Dict[str, Any] = {
        ...
        "world": world,
        "ego": dict(to_scene_ego_from_car_footprint(CarFootprint.build_from_center(ego_pose, get_mine_truck_parameters()))),
    }
