...
def update_config_for_simulation(cfg: DictConfig) -> None:
    ...
    # if cfg.log_config:
    if False:
        logger.info(f"#log# Creating experiment: {cfg.experiment}")
...
def update_distributed_optimizer_config(cfg: DictConfig) -> DictConfig:
    ...
    logger.info(f"#log# World size: {lr_scale}")
    logger.info(f"#log# Learning rate before: {cfg.optimizer.lr}")
    scaling_method = "Equal Variance" if cfg.lightning.distributed_training.equal_variance_scaling_strategy else "Linearly"
    logger.info(f"#log# Scaling method: {scaling_method}")
    ...
    if is_target_type(cfg.optimizer, torch.optim.Adam) or is_target_type(cfg.optimizer, torch.optim.AdamW):
        ...
        logger.info(f"#log# Betas after scaling: {cfg.optimizer.betas}")

    logger.info(f"#log# Learning rate after scaling: {cfg.optimizer.lr}")

    return cfg


def scale_parameter(parameter: float, world_size: int, equal_variance_scaling_strategy: bool, raise_power: bool = False) -> float:
    """
...
def get_num_gpus_used(cfg: DictConfig) -> int:
    ...
    if num_gpus == -1:  # if environment variable WORLD_SIZE is not set, find from trainer
        logger.info("#log# WORLD_SIZE was not set.")
        ...
        if trainer_num_gpus == -1:  # if trainer gpus = -1, all gpus are used, so find all available devices
            logger.info("#log# PytorchLightning Trainer gpus was set to -1, finding number of GPUs used from torch.cuda.device_count().")
            cuda_num_gpus = torch.cuda.device_count() * int(os.getenv("NUM_NODES", 1))
            num_gpus = cuda_num_gpus

        else:  # if trainer gpus is not -1
            logger.info(f"#log# Trainer gpus was set to {trainer_num_gpus}, using this as the number of gpus.")
            num_gpus = trainer_num_gpus

    num_gpus = int(num_gpus)
    logger.info(f"#log# Number of gpus found to be in use: {num_gpus}")
    return num_gpus
