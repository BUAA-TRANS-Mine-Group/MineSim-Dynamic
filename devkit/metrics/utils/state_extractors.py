...
def approximate_derivatives(
    y: npt.NDArray[np.float32], x: npt.NDArray[np.float32], window_length: int = 5, poly_order: int = 2, deriv_order: int = 1, axis: int = -1
) -> npt.NDArray[np.float32]:
    """
    Given two equal-length sequences y and x, compute an approximation to the n-th derivative of some function interpolating the (x, y) data points, and return its
...
def extract_ego_acceleration(
    ego_states: List[EgoState], acceleration_coordinate: str, decimals: int = 8, poly_order: int = 2, window_length: int = 8
) -> npt.NDArray[np.float32]:
    """
    ...
    if acceleration_coordinate == "x":
        ...
        raise ValueError(f"acceleration_coordinate option: {acceleration_coordinate} not available. " f"Available options are: x, y or magnitude")
...
def extract_ego_jerk(
    ego_states: List[EgoState], acceleration_coordinate: str, decimals: int = 8, deriv_order: int = 1, poly_order: int = 2, window_length: int = 15
) -> npt.NDArray[np.float32]:
    """
...
def extract_ego_yaw_rate(
    ego_states: List[EgoState], deriv_order: int = 1, poly_order: int = 2, decimals: int = 8, window_length: int = 15
) -> npt.NDArray[np.float32]:
    """
    Extract ego rates
    :param ego_states: A list of ego states
...
def extract_tracks_poses(history: SimulationHistory) -> List[npt.NDArray[np.float32]]:
    ...
    try:
        for sample in history.data:
            poses: List[npt.NDArray[np.float32]] = [np.array([*tracked_object.center]) for tracked_object in sample.observation.tracked_objects]
