...
from devkit.common.utils.s3_utils import get_s3_client
...
class FileBackedBarrier:
    ...
    def wait_barrier(
        ...
        """
        logger.info("#log# Writing completion of activity id %s to directory %s...", activity_id, self._barrier_directory)
        self._register_activity_id_complete(activity_id)

        logger.info("#log# Waiting for all processes to finish processing")
        self._wait(expected_activity_ids, timeout_s, poll_interval_s)

        logger.info(
            f"#log# Sleeping for {poll_interval_s * SLEEP_MULTIPLIER_BEFORE_CLEANUP} seconds so that the other processes catch up before moving on"
        )
        time.sleep(poll_interval_s * SLEEP_MULTIPLIER_BEFORE_CLEANUP)

        logger.info("#log# All Processes Synced, clearing activity file")
        self._remove_activity_after_processing(activity_id)

        logger.info("#log# Waiting for all processes to clean up barrier files")
    ...
    def _wait(self, expected_activity_ids: Set[str], timeout_s: Optional[float] = None, poll_interval_s: float = 1) -> None:
        ...
        logger.info("#log# Beginning barrier wait at time %f", start_wait_time)
        while True:
            next_wait_time = time.time() + poll_interval_s
            logger.debug("#log# The next wait time is %f. Getting completed activity ids...", next_wait_time)
            completed_activity_ids = self._get_completed_activity_ids()

            logger.debug("#log# There are %d completed activities.", len(completed_activity_ids))
            ...
            logger.debug("#log# All tasks not finished. Total elapsed wait time is %f.", total_wait_time)
            ...
            logger.debug("#log# Sleeping for %f seconds.", sleep_time)
    ...
    def _create_activity_file_in_s3(self, s3_key: Path, s3_bucket: str) -> None:
        ...
        with closing(get_s3_client()) as s3_client:
            logger.info(f"#log# Creating activity file at {s3_key} in bucket {s3_bucket}...")
    ...
    def _remove_activity_file_from_s3(self, s3_key: Path, s3_bucket: str) -> None:
        ...
        """
        with closing(get_s3_client()) as s3_client:
            logger.info(f"#log# Removing activity file at {s3_key} in bucket {s3_bucket}...")
