...
def _get_session_internal(
    ...
    if os.getenv("AWS_WEB_IDENTITY_TOKEN_FILE") is not None:
        logger.debug("#log# Using AWS_WEB_IDENTITY_TOKEN_FILE for credentials.")
    ...
    # This would be typically used with AWS SSO.
        ...
        logger.debug("#log# Using default credentials for AWS session.")

    # If profile / key information is provided, then attempt to use that for the credentials.
    else:
        logger.debug("#log# Attempting to use credentialed authentication for S3 client...")
    ...
    try:
        ...
        if "profile_name" in args:
            logger.info(
                "#log# Trying default AWS credential chain, since we got this exception "
...
async def download_file_from_s3_async(local_path: Path, s3_key: Path, s3_bucket: str) -> None:
    ...
    async with session.client("s3") as async_s3_client:
        logger.info(f"#log# Downloading {s3_key} to {local_path} in bucket {s3_bucket}...")
...
def expand_s3_dir(
    ...
    """
    logger.warning("#log# Function expand_s3_dir will soon be removed in favor of list_files_in_s3_directory")
