...
class StateSE2(Point2D):
    ...
    **SE(2)** 指的是 **Special Euclidean Group in 2 dimensions**，表示二维空间中的刚体运动。它包含两个部分：
        1. **平移 (Translation)**：物体在平面上的平移操作，通常用二维坐标 [x, y] 表示。
        2. **旋转 (Rotation)**：物体在平面上的旋转操作，通常用一个角度（heading，或 yaw）表示。旋转是在二维平面上绕着 z 轴（垂直于平面）的旋转。
    """
...
class ProgressStateSE2(StateSE2):
    ...
    progress: float  # [m] distance along a path 沿路径起点的距离
    __slots__ = "progress"
...
class StateVector2D:
    ...
    - 注意:
        - 表示ego车的速度\加速度Vector.定位点通常在后轴中心;
        - 表示agent车的速度\加速度Vector.定位点通常在几何中心;
        - 车体坐标系:【 右手坐标系，x-纵向 向前为正，y-横向 向左为正】
        - e.g. 计算得到更新后的后轴加速度。
        from devkit.sim_engine.ego_simulation.ego_update_model.kinematic_bicycle_model_response_lag import KinematicBicycleModelResponseLa
    """
