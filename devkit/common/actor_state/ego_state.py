...
from devkit.common.actor_state.dynamic_car_state import DynamicCarState, get_acceleration_shifted, get_velocity_shifted
...
class EgoState(InterpolatableState):
    ...
    def deserialize(vector: List[Union[int, float]], vehicle: VehicleParameters) -> EgoState:
        ...
        """
        if len(vector) != 9:
            raise RuntimeError(f"#log# Expected a vector of size 9, got {len(vector)}")
    ...
    def from_split_state(split_state: SplitState) -> EgoState:
        ...
        return EgoState.build_from_rear_axle(
            rear_axle_pose=StateSE2(split_state.linear_states[1], split_state.linear_states[2], split_state.angular_states[0]),
    ...
    def build_from_rear_axle(
        ...
        使用自车后轴的原始参数初始化 EgoState 实例，默认参考系为 CAR_POINT.REAR_AXLE。
        :param rear_axle_pose: 自车后轴的位姿（StateSE2 对象）
        :param rear_axle_velocity_2d: 自车后轴的速度向量（StateVector2D 对象）
        :param rear_axle_acceleration_2d: 自车后轴的加速度向量（StateVector2D 对象）
        :param tire_steering_angle: 车轮转向角度
        :param time_point: 自车状态的时间戳
        :param vehicle_parameters: 车辆参数
        :param is_in_auto_mode: 如果自车处于自动模式则为 `True`，否则为 `False`（默认为 `True`）
        :param angular_vel: 自车的角速度（默认值为 0.0）;逆时针为正
        :param angular_accel: 自车的角加速度（默认值为 0.0）
        :param tire_steering_rate: 车轮的转向速度（单位：弧度/秒，默认值为 0.0）
        :return: 初始化的 `EgoState` 对象
        """
