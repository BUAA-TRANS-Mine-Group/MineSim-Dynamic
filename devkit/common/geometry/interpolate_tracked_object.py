...
from devkit.common.geometry.interpolate_state import interpolate_future_waypoints, interpolate_past_waypoints
...
def interpolate_agent(agent: AgentTemporalState, horizon_len_s: float, interval_s: float) -> AgentTemporalState:
    ...
    if interpolated_agent.predictions:
        interpolated_agent.predictions = [
            PredictedTrajectory(
                waypoints=interpolate_future_waypoints(mode.waypoints, horizon_len_s=horizon_len_s, interval_s=interval_s),
    ...
    if past_trajectory:
        interpolated_agent.past_trajectory = PredictedTrajectory(
            waypoints=interpolate_past_waypoints(past_trajectory.waypoints, horizon_len_s=horizon_len_s, interval_s=interval_s),
...
def interpolate_tracks(tracked_objects: Union[TrackedObjects, List[TrackedObject]], horizon_len_s: float, interval_s: float) -> List[TrackedObject]:
    ...
    return [interpolate_agent(agent, horizon_len_s=horizon_len_s, interval_s=interval_s) for agent in all_tracked_objects.get_agents()] + cast(
        List[TrackedObject], all_tracked_objects.get_static_objects()
    )
